...
const findBiomeConfig = (startDir, fs, path) => Effect.gen(function* () {
    let dir = startDir;
...
const getRelativePluginPaths = () => {
    return PLUGINS.map((p) => `./node_modules/${PLUGIN_NAMESPACE}/${p}`);
};
const hasPlugins = (config) => {
    if (!Array.isArray(config.plugins))
        return false;
    return PLUGINS.every((p) => config.plugins.some((plugin) => typeof plugin === "string" && plugin.includes(p)));
};
const addPlugins = (config, pluginPaths) => ({
    ...config,
    plugins: [...(config.plugins || []), ...pluginPaths],
});
// --- Dry Run ---
const dryRun = (biomePath, pluginPaths) => Effect.gen(function* () {
    if (biomePath === null) {
...
// --- Apply ---
const apply = (biomePath, pluginPaths, fs, path) => Effect.gen(function* () {
    if (biomePath === null) {
...
}, ({ dryRun: isDryRun }) => Effect.gen(function* () {
...
cli(process.argv).pipe(Effect.provide(NodeContext.layer), Effect.tapError((error) => Console.error(`\n  ${formatError(error)}\n`)), Effect.catchAll(() => Effect.void), NodeRuntime.runMain);
