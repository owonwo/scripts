...
const findBiomeConfig = (startDir, fs, path) =>
  Effect.gen(function* () {
    let dir = startDir;
...
const hasPlugins = (config) => {
  ...
  return PLUGINS.every((p) =>
    config.plugins.some((plugin) => typeof plugin === "string" && plugin.includes(p)),
  );
};
...
// --- Dry Run ---
const dryRun = (biomePath, pluginPaths) =>
  Effect.gen(function* () {
    if (biomePath === null) {
...
// --- Apply ---
const apply = (biomePath, pluginPaths, fs, path) =>
  Effect.gen(function* () {
    if (biomePath === null) {
