...
export async function loadAndFilterFiles(
  ...
  force: boolean,
  debug: boolean
): Promise<{ filesToProcess: string[]; cachedResults: { filePath: string; transformed: boolean; componentsFound: number }[] }> {
  ...
  try {
    if (session.status === "upToDate") {
      ...
      const payload: CachePayload | null = session.compressedPayloads.length > 0
