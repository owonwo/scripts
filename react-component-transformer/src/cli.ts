...
function filterTsxFiles(files: string[]): string[] {
  return files.filter(
    (file) =>
      file.endsWith(".tsx") &&
...
const command = Command.make(
  ...
  ({ directory, workers }) =>
    Effect.gen(function* () {
      ...
      const files = filterTsxFiles(allFiles).map((file) =>
      ...
      yield* Console.log(
        `Found ${files.length} .tsx files. Transforming with ${workers} workers...`
      );
        ...
        Effect.gen(function* () {
          return yield* Effect.async<TransformResult[], never>((resume) => {
            ...
            worker.on("message", (result: unknown) => {
              workerResults.push(result as TransformResult);
              completed++;
